...
local g_FitTxt;
--- 蜜语金丝对应蜜语
local g_supertooltip2_sweet_words = {
...
local g_supertooltip2_space_num = {
    [1]  = { spaceStr = "             - "},
    [2]  = { spaceStr = "            - "},
    [3]  = { spaceStr = "           - "},
    [4]  = { spaceStr = "          - "},
...
local sdl = 0;
--local sldbfbjc={28,34,40,46,54,63,72,83,95}--这个其实没什么用了，要百分比干什么呢？蝎子注
...
function SuperTooltip2_OnLoad()
	...
	g_FitTitle = SuperTooltip2_StaticPart_QH1;
	g_FitTxt = SuperTooltip2_StaticPart_QH2;
	g_FitTitle:Hide();
	g_FitTxt:Hide();
...
function SuperTooltip2_OnEvent(event)
	...
	if (event == "SHOW_SUPERTOOLTIP2") then
		if ( arg0 == "1" and SuperTooltips2:IsPresent()) then
			...
			_SuperTooltip2_:SetProperty("AbsoluteXPosition",0)
			_SuperTooltip2_:SetProperty("AbsoluteYPosition",0)
			...
			--------显示装备比较窗口---------Start
			local nCmpWndCount = tonumber(arg4)
			local posX = SuperTooltip2_Frame:GetProperty("AbsoluteXPosition")
			local posY = SuperTooltip2_Frame:GetProperty("AbsoluteYPosition")
			local nWidth = SuperTooltip2_Frame:GetProperty("AbsoluteWidth")
			--SuperTooltips2:ShowCmp2WindowMain( tonumber(posX) ,tonumber(posY) ,tonumber(nWidth) ,nCmpWndCount )
			--------显示装备比较窗口---------End
			this:Show();
			return;
		elseif arg0 == "2" and SuperTooltips2:IsPresent() and this:IsVisible() then
			--- 这种情况是，按下Alt键，要显示聊天频道的装备属性对比
			SuperTooltips2:ShowToolTips2WithAlt( 1 )
			return
		elseif arg0 == "3" and SuperTooltips2:IsPresent() and this:IsVisible() then
			--- 这种情况是：抬起Alt键，要关闭聊天频道的装备属性对比
			SuperTooltips2:ShowToolTips2WithAlt( 0 )
			return
		else
			this:Hide();
			return;
...
function SuperTooltip2_Update()
	...
	typeDesc = SuperTooltips2:GetTypeDesc();
	...
	szPropertys = SuperTooltips2:GetPropertys();
	local szExplain = SuperTooltips2:GetExplain();
	local nYuanbaotrade = SuperTooltips2:GetYuanbaoTradeFlag();
	local nGoodsProtect = 0;
	local nEquipMasterFlag = 0;--SuperTooltips2:GetEquipMasterFlag();
	local nEquiMasterText = 0;--SuperTooltips2:GetEquipMasterText();
	local nBaiShouLevelText = 0;--SuperTooltips2:GetBaiShouLevelText();
	szAuthor1 = SuperTooltips2:GetAuthorInfo();
	szAuthor = SuperTooltips2:GetAuthorInfo();
	bspf = GetBaoshipingfen(szAuthor)
	local hehed,heheD = 0,0
	if  szAuthor ~= "" and szAuthor ~=nil   then
		local sz,aswq,hehed,heheD = string.find(szAuthor,"&SL(%d)(%d%d)")
		
		if sz ~= nil or aswq ~= nil then
			hehed = tonumber(hehed)
			if hehed ==nil then hehed =0  end
			heheD = tonumber(heheD)
			if heheD ==nil then heheD =0   end
		end
	end
	local Baoshipftotal,baoshipfStr = 0,szPropertys
	local ItemID = SuperTooltips2:GetTitle()
	...
	if nGemHoleCounts ~= nil then
		...
		if  nGemHoleCountstable[nGemHoleCounts] then
			if szPropertys~=nil then
				Baoshipftotal = GetBaoshiStrrBB(szPropertys,nGemHoleCountstable[nGemHoleCounts])
			end
		end
	end
	...
	local infantpos1,infantxpos1 = string.find(SuperTooltips2:GetTitle(), "宝珠·") -------令牌宝珠摘除处理等级变化
	if  infantpos1 ~= nil and  infantxpos1 ~= nil then
		if szAuthor~=nil then
			local starpos1,endpos1,bzdj = string.find(szAuthor,"BZ(%w%w)")
			if starpos1 ~= nil and  endpos1 ~= nil then
				SuperTooltip2_StaticPart_Title:SetText(SuperTooltips2:GetTitle()..tonumber(bzdj).."级")
			end
		end
	end
	...
	if Baoshipftotal >= 15 then
		toDisplay = toDisplay .. ";SuperTooltip2_Notice";
	...
	if (szAuthor ~= nil) then
		if typeDesc == "#cffcc99帽子" or typeDesc == "#cffcc99鞋" or typeDesc == "#cffcc99护肩" or typeDesc == "#cffcc99腰带" or typeDesc == "#cffcc99手套" or typeDesc == "#cffcc99护符" or typeDesc == "#cffcc99戒指" or typeDesc == "#cffcc99项链"  or typeDesc == "#cffcc99护腕" or typeDesc == "#cffcc99衣服"  then
			...
			if strD and strD >3 then
				...
				SuperTooltip2_Manufacturer:SetText(strName);
			else
 ...
 -------************************************
	-------—本作品由君奉原创独家，仅作为个人交流使用，请勿开服外传，本人不承担任何法律责任— 王权装备处理
	-------************************************
	...
	SuperTooltip2_StaticPart_Title:Show()
	SuperTooltip2_StaticPart_Title1:Hide()
	SuperTooltip2_StaticPart_Title2:Hide()
	SuperTooltip2_StaticPart_Animate:Hide()
	SuperTooltip2_StaticPart_ZhiZun:SetText("")
	if  szPropertys ~= nil and szAuthor ~=nil and is_shengling ==1 then
		...
		if  starpos1 ~= nil  and  endpos1~=nil  then
			...
			wqdj =tonumber(wqdj)
            if wqtype==1 then
				SuperTooltip2_StaticPart_Title:Hide()
				SuperTooltip2_StaticPart_Title1:Show()
				SuperTooltip2_StaticPart_Title2:Hide()
				SuperTooltip2_StaticPart_Title1:SetText("#b#cff6633【王权】"..SuperTooltips2:GetTitle())
				SuperTooltip2_StaticPart_Animate:Show()
				SuperTooltip2_StaticPart_Animate:SetProperty("Animate" , "ZhiZun_Flash_Small")
			end
			if wqtype==1 and wqdj==0 then
				SuperTooltip2_StaticPart_ZhiZun:SetText("#H王权：(+"..sl_cs[wqtype][wqdj].."%)"  )
			elseif  wqtype==2 and wqdj==0 then
				SuperTooltip2_StaticPart_Title:Hide()
				SuperTooltip2_StaticPart_Title1:Hide()
				SuperTooltip2_StaticPart_Title2:Show()
				SuperTooltip2_StaticPart_Title2:SetText("#b#cff6633【天道】"..SuperTooltips2:GetTitle())
				SuperTooltip2_StaticPart_Animate:Show()
				SuperTooltip2_StaticPart_Animate:SetProperty("Animate" , "SuperEquip_Big")
				SuperTooltip2_StaticPart_ZhiZun:SetText("#H天道：(+"..sl_cs[wqtype][wqdj].."%)"  )
			elseif  wqtype>=0 and wqdj>0 then
				SuperTooltip2_StaticPart_ZhiZun:SetText("#H升灵度:#Y"..wqdj.."/9#H(+"..sl_cs[wqtype][wqdj].."%)"  )
			end
	...
	--SuperTooltip2_StaticPart_Item4:SetText(SuperTooltips2:GetDesc4());
            ...
            local strong = tonumber(QiangHuaBBBB())

			local szAuthor1 = SuperTooltips2:GetAuthorInfo();	
if 	szAuthor1 ~= nil then 
    local _,t1 = string.find(szAuthor1,"PYY")
	 if t1 ~= nil then
					if strong ~= nil then					
			        if strong > 8 then
					SuperTooltip2_StaticPart_Item4:SetText("#c0FFFFF强化: +"..tonumber(strong));					
					
					local Strongs1,Strongx1 = string.find(typeDesc, "衣服")
					if  Strongs1 ~= nil and Strongx1 ~= nil then
					SuperTooltip2_StaticPart_Item6:SetText("#c0FFFFF穿刺减免: +"..g_ExpTbl[tonumber(strong)]/2)
					else
					SuperTooltip2_StaticPart_Item6:SetText("#c0FFFFF穿刺伤害: +"..g_ExpTbl[tonumber(strong)])
					end
			...
			for i = 0,16 do
				for j = 17,22 do
					...
					if Strongs ~= nil and Strongx ~= nil then
						...
						SuperTooltip2_StaticPart_Item6:SetText("#c0FFFFF穿刺伤害: +"..g_ExpTbl[AddSuansi])
					end
					if  Strongs1 ~= nil and Strongx1 ~= nil then
						qianghu1 = 1
						SuperTooltip2_StaticPart_Item4:SetText("#c0FFFFF强化: +"..SuperTooltips2:GetDesc4());
						SuperTooltip2_StaticPart_Item6:SetText("#c0FFFFF穿刺减免: +"..g_ExpTbl[AddSuansi]/2)
					end
				end
			end
	...
	--SuperTooltip2_StaticPart_Item6:SetText(SuperTooltips2:GetDesc6());
	...
	if (mynamew ~= 0 and mynamew ~= nil ) then
		...
		SuperTooltip2_StaticPart_DW:SetProperty("Image", mynamew)
	else
		SuperTooltip2_StaticPart_DW:Hide()
	end
	if (fr ~= 0 and fr ~= nil) then
		SuperTooltip2_StaticPart_DW1:Show()
		SuperTooltip2_StaticPart_DW1:SetProperty("Image", fr)
	else
		SuperTooltip2_StaticPart_DW1:Hide()
	end
	...
	--else
		--SuperTooltip2_Protect_Text:SetText("")
	      ...
	      -- local OldLevel = SuperTooltips2:GetDesc1()
	 ...
	 -- local _,level = string.find(OldLevel,"等级:")
		 ...
		 -- if NewLevel >= 20 then
		 -- SuperTooltip2_BaiBingJingTong_Text:SetText("#c00ccff携带等级降至".. NewLevel-10 .. "（百兵精通）#cFF0000后期更新")
		 -- else
		 -- SuperTooltip2_BaiBingJingTong_Text:SetText("")
		 -- end
		  -- end
	 -- end
	...
	if (qual ~= nil)then
		local nByQual = tonumber(qual)
		local starNum = 0;
		local nFit = 0;
		if nByQual > 10 then
			starNum = math.floor(nByQual / 10);
			nFit = math.mod(nByQual, 10)
			if starNum < 8 then
				g_FitTxt:SetText("" .. nFit .. "/9")
				g_FitTitle:Show();
				g_FitTxt:Show();
			else
				g_FitTitle:Hide();
	...
	if ( szPropertys ~= nil)  and (hehed ~= 0 ) then
		-----下面是解析模块
		...
		for i = 1 ,table.getn(g_nContentButton) do
			if g_nContentButton[i] ~= nil then
				...
				if szaswq ~= nil then
					...
					str =str..strjbz.." #H+"..(g_QWsx[(math.mod(tonumber(hehed),10))]).."#r"
					str3 = str3..strjbz.."#r"
				end
			end
		...
		if szaswq ~= nil then
			...
			local zhongguou= ahehed.."#r".. str.."#r".. aheheed.."#r"
			for y = 1 ,16 do
				local yyu = string.find(zhongguou,"#r#r")
				
				if yyu ~= nil then
					zhongguou =string.gsub(zhongguou, "#r#r","#r");
				end
				
			end
			szPropertys = zhongguou
	...
	if ( szPropertys ~= nil) then
		...
		if  zhenyuanpos1 ~= nil and  zhenyuanxpos1 ~= nil then
			...
			SuperTooltip2_ShortDesc_Text:SetText("#cffcc99真元")
		end
	end
...
if cy01n ~= nil then
	...
	local shuxa = math.floor(tonumber(shux) * 1.95)
		if cy03n ~= nil then
...
if cy01n ~= nil then
	...
	local shuxb = math.floor(tonumber(shuxxx) * 1.95)
		if cy03n ~= nil then
...
if cy01n ~= nil then
	...
	if ( szPropertys ~= nil) then
		...
		if sPos ~= nil and ePos ~= nil then
			SuperTooltip2_StaticPart_Item2:SetText("#cC8B88E耐久:200/200");
			SuperTooltip2_StaticPart_Item3:SetText("#cC8B88E可修理次数:3/3");
				-- for i = 1,9 do
				-- g_Stars[i]:SetProperty("Animate", "Animate_Star");
				-- g_Stars[i]:Show();
				-- end			
			local  _,art = DisplayLPInfoo()
			local sPos1, ePos1 = string.find(szPropertys, "# LLemId=")
			if sPos1 ~=nil and ePos1 ~= nil then
				local str = string.sub(szPropertys,ePos1+1,ePos1+8)
				szPropertys= string.gsub(szPropertys,"# LLemId="..str,""..art)
			end
			local lllla = DODPDIEdddd()
...
---这里完善时装上面的宝石信息显示		
		...
		gemIcon = SuperTooltips2:GetGemIcon2();--第二个点缀
		if (gemIcon ~= "") then
		BaoShiName = JsMiYu(gemIcon)
		szPropertys = szPropertys..BaoShiName			
		end
		
		gemIcon = SuperTooltips2:GetGemIcon3();--第三个点缀
		if (gemIcon ~= "") then
		BaoShiName = JsMiYu(gemIcon)
...
szPropertys = szPropertys.."#{MYJS_120723_25}\n"..g_supertooltip2_sweet_words[myql].sweetWords..cy04 
else
szPropertys = szPropertys
end
end
	...
	if ( szPropertys ~= nil) then
		...
		if sPos ~=nil and ePos ~= nil then
			...
			szPropertys = string.gsub(szPropertys,"# QTemId="..str,xieziQL..str3)
				...
				SuperTooltip2_Property:SetText(szPropertys.."#G#{BSZJPF_130311_03} +"..Baoshipftotal.."#r#G#{BSZJPF_130311_04} +"..Baoshipftotal);
			else
	...
	if ( szPropertys ~= nil) then
		...
		if sPos ~=nil and ePos ~= nil then
			local str = string.sub(szPropertys,ePos+1,ePos+8)
			local a,b, JTstr1,JTstr2,JTstr3 = GDisplEQJTInfoo(szAuthor)
			if a ==0 or b ==0 then
	...
	if( szPropertys ~= nil) and string.find(typeDesc, "暗器") ~=nil then  --暗器特写
		...
		if sPos ~=nil and ePos ~= nil then
			...
			SuperTooltip2_StaticPart_DW1:Show()
			SuperTooltip2_StaticPart_DW1:SetProperty("Image", "set:TattooShow image:TattooShow_1")
			SuperTooltip2_Property:SetText("#cFF0000"..ss.."#r"..str1);
		else
			SuperTooltip2_Property:SetText(szPropertys);
			SuperTooltip2_StaticPart_DW:Hide()
		end
	...
	if ( szPropertys ~= nil) then
		...
		if sPos ~=nil and ePos ~= nil then
			...
			SuperTooltip2_StaticPart_Item1:SetText("#cC8B88E等级: "..PetLev);
			for i=1,PetStar do
				if PetStar <=4 then
					g_Stars[i]:SetProperty("Animate", "Animate_StarNoFlash");
				else
			...
			if PetStar < 6 then
				SuperTooltip2_StaticPart_Item2:SetText("#G可拆解");
				SuperTooltip2_StaticPart_Item3:SetText("#G可提升星级");
			else
				SuperTooltip2_StaticPart_Item2:SetText("#cff6633可拆解");
				SuperTooltip2_StaticPart_Item3:SetText("#cff6633已满级");
			end
			
			if PetType == 1 then
				SuperTooltip2_ShortDesc_Text:SetText("#cC8B88E珍兽面甲"); --显示类型
			elseif PetType == 2 then
				SuperTooltip2_ShortDesc_Text:SetText("#cC8B88E珍兽武器"); --显示类型
			elseif PetType == 3 then
				SuperTooltip2_ShortDesc_Text:SetText("#cC8B88E珍兽盔甲"); --显示类型
			elseif PetType == 4 then
				SuperTooltip2_ShortDesc_Text:SetText("#cC8B88E珍兽项圈"); --显示类型
			else
				SuperTooltip2_ShortDesc_Text:SetText("#cC8B88E珍兽配饰"); --显示类型
			end
			
			
			if PetLev == 75 then
				local petEqJCSX = {"#cC8B88E基础外功防御 +580#r基础内功防御 +580","#cC8B88E基础外功攻击 +587#r基础内功攻击 +587","#cC8B88E基础闪避 +531"}  --基础属性
				local petEqEWSX = {"#r#cffcc99内功攻击 +"..tostring(PetStar*172).."#r会心 +"..tostring(PetStar*3).."","#r#cffcc99外功攻击 +"..tostring(PetStar*172).."#r命中 +"..tostring(PetStar*24).."","#r#cffcc99外功防御 +"..tostring(PetStar*100).."#r内功防御 +"..tostring(PetStar*171)..""}  --额外属性
				local petEqQHSX = {"#r#c0066ff灵气资质契合：灵气 +"..tostring(PetStar)..".0%","#r#c0066ff力量资质契合：力量 +"..tostring(PetStar)..".0%","#r#c0066ff体力资质契合：体力 +"..tostring(PetStar)..".0%"}
				local PetEqNam = {"黄雀戏水·怯","苍狼啸月·勇","乌豚望日·忠"}
				local PetEqShuX = {}
				PetEqShuX[1] = {"黄雀戏水兽盔·怯","黄雀戏水兽爪·怯","黄雀戏水兽甲·怯","黄雀戏水兽环·怯","苍狼啸月兽饰·怯"}
				PetEqShuX[2] = {"苍狼啸月兽盔·勇","苍狼啸月兽爪·勇","苍狼啸月兽甲·勇","苍狼啸月兽环·勇","苍狼啸月兽饰·勇"}
				PetEqShuX[3] = {"乌豚望日兽盔·忠","乌豚望日兽爪·忠","乌豚望日兽甲·忠","乌豚望日兽环·忠","乌豚望日兽饰·忠"}
				local PetEqYCSX = {}
				PetEqYCSX[1] = {"#r#c33cc99内功攻击 +"..tostring(PetStar*332).."","#r#c33cc99命中 +"..tostring(PetStar*430).."","#r#c33cc99灵气 +"..tostring(PetStar*17).."","#r#c33cc99烈火咒、玄雷咒、血毒咒、寒冰咒释放时额外增加对应属性攻击("..tostring(PetStar).."星套效果)"}
				PetEqYCSX[2] = {"#r#c33cc99外功攻击 +"..tostring(PetStar*332).."","#r#c33cc99命中 +"..tostring(PetStar*430).."","#r#c33cc99力量 +"..tostring(PetStar*17).."","#r#c33cc99猛击技能释放时额外增加外功攻击("..tostring(PetStar).."星套效果)"}
				PetEqYCSX[3] = {"#r#c33cc99外功防御 +"..tostring(PetStar*328).."","#r#c33cc99内功防御 +"..tostring(PetStar*328).."","#r#c33cc99血上限 +"..tostring(PetStar*609).."","#r#c33cc99增加灵动技能对珍兽自己的生效几率("..tostring(PetStar).."星套效果)"}
				...
				for i = 1,5 do
					if PetStar == math.floor(math.mod(PetEq[i],100)/10) and PetType == math.floor(math.mod(PetEq[i],1000)/100) then
						...
						TaoNum = 0
						TaoEvery = "#r#ccccccc"..PetEqShuX[PetType][i]
					end
				...
				szPropertys = "#cccffff已绑定".."#r"..TaoJCSX..PetTaoNam..TaoHave..TaoYCSX.."#r#P资质未鉴定"
				SuperTooltip2_Property:SetText(szPropertys);
			end
			
			
			if PetLev == 85 then
				local petEqJCSX = {"#cC8B88E基础外功防御 +649#r基础内功防御 +649","#cC8B88E基础外功攻击 +657#r基础内功攻击 +657","#cC8B88E基础闪避 +593"}  --基础属性
				local petEqEWSX = {"#r#cffcc99内功攻击 +"..tostring(PetStar*192).."#r命中 +"..tostring(PetStar*26).."#r会心 +"..tostring(PetStar*4).."","#r#cffcc99外功攻击 +"..tostring(PetStar*192).."#r命中 +"..tostring(PetStar*26).."#r会心 +"..tostring(PetStar*4).."","#r#cffcc99外功防御 +"..tostring(PetStar*190).."#r内功防御 +"..tostring(PetStar*190).."#r闪避 +"..tostring(PetStar*12)..""}  --额外属性
				local petEqQHSX = {"#r#c0066ff灵气资质契合：灵气 +"..tostring(PetStar*2)..".0%","#r#c0066ff力量资质契合：力量 +"..tostring(PetStar*2)..".0%","#r#c0066ff体力资质契合：体力 +"..tostring(PetStar*2)..".0%"}
				local PetEqNam = {"飞鹰翔空·怯","猛虎撼山·勇","巨熊哮路·忠"}
				local PetEqShuX = {}
				PetEqShuX[1] = {"飞鹰翔空兽盔·怯","飞鹰翔空兽爪·怯","飞鹰翔空兽甲·怯","飞鹰翔空兽环·怯","猛虎撼山兽饰·怯"}
				PetEqShuX[2] = {"猛虎撼山兽盔·勇","猛虎撼山兽爪·勇","猛虎撼山兽甲·勇","猛虎撼山兽环·勇","猛虎撼山兽饰·勇"}
				PetEqShuX[3] = {"巨熊哮路兽盔·忠","巨熊哮路兽爪·忠","巨熊哮路兽甲·忠","巨熊哮路兽环·忠","巨熊哮路兽饰·忠"}
				local PetEqYCSX = {}
				PetEqYCSX[1] = {"#r#c33cc99内功攻击 +"..tostring(PetStar*374).."","#r#c33cc99命中 +"..tostring(PetStar*483).."","#r#c33cc99灵气 +"..tostring(PetStar*19).."","#r#c33cc99痛击技能释放时额外增加内功攻击("..tostring(PetStar).."星套效果)"}
				PetEqYCSX[2] = {"#r#c33cc99外功攻击 +"..tostring(PetStar*374).."","#r#c33cc99命中 +"..tostring(PetStar*483).."","#r#c33cc99力量 +"..tostring(PetStar*19).."","#r#c33cc99增加猛击技能的释放几率("..tostring(PetStar).."星套效果)"}
				PetEqYCSX[3] = {"#r#c33cc99外功防御 +"..tostring(PetStar*369).."","#r#c33cc99内功防御 +"..tostring(PetStar*369).."","#r#c33cc99血上限 +"..tostring(PetStar*667).."","#r#c33cc99增加忠心技能对玩家的生效几率("..tostring(PetStar).."星套效果)"}
			...
			if PetLev == 95 then
				local petEqJCSX = {"#cC8B88E基础外功防御 +718#r基础内功防御 +718","#cC8B88E基础外功攻击 +727#r基础内功攻击 +727","#cC8B88E基础闪避 +655"}  --基础属性
				local petEqEWSX = {"#r#cffcc99内功攻击 +"..tostring(PetStar*211).."#r命中 +"..tostring(PetStar*29).."#r会心 +"..tostring(PetStar*5).."#r灵气 +"..tostring(PetStar*10).."","#r#cffcc99外功攻击 +"..tostring(PetStar*211).."#r命中 +"..tostring(PetStar*29).."#r会心 +"..tostring(PetStar*5).."#r力量 +"..tostring(PetStar*10).."","#r#cffcc99外功防御 +"..tostring(PetStar*209).."#r内功防御 +"..tostring(PetStar*209).."#r闪避 +"..tostring(PetStar*15).."#r体力 +"..tostring(PetStar*10)..""}  --额外属性
				local petEqQHSX = {"#r#c0066ff灵气资质契合：灵气 +"..tostring(PetStar*3)..".0%","#r#c0066ff力量资质契合：力量 +"..tostring(PetStar*3)..".0%","#r#c0066ff体力资质契合：体力 +"..tostring(PetStar*3)..".0%"}
				local PetEqNam = {"昆鹏异羽·怯","雄狮逆鳞·勇","玄龟奇血·忠"}
				local PetEqShuX = {}
				PetEqShuX[1] = {"昆鹏异羽兽盔·怯","昆鹏异羽兽爪·怯","昆鹏异羽兽甲·怯","昆鹏异羽兽环·怯","雄狮逆鳞兽饰·怯"}
				PetEqShuX[2] = {"雄狮逆鳞兽盔·勇","雄狮逆鳞兽爪·勇","雄狮逆鳞兽甲·勇","雄狮逆鳞兽环·勇","雄狮逆鳞兽饰·勇"}
				PetEqShuX[3] = {"玄龟奇血兽盔·忠","玄龟奇血兽爪·忠","玄龟奇血兽甲·忠","玄龟奇血兽环·忠","玄龟奇血兽饰·忠"}
				local PetEqYCSX = {}
				PetEqYCSX[1] = {"#r#c33cc99内功攻击 +"..tostring(PetStar*416).."","#r#c33cc99命中 +"..tostring(PetStar*536).."","#r#c33cc99灵气 +"..tostring(PetStar*21).."","#r#c33cc99增加烈火咒、玄雷咒、血毒咒、寒冰咒释放几率("..tostring(PetStar).."星套效果)"}
				PetEqYCSX[2] = {"#r#c33cc99外功攻击 +"..tostring(PetStar*416).."","#r#c33cc99命中 +"..tostring(PetStar*536).."","#r#c33cc99力量 +"..tostring(PetStar*21).."","#r#c33cc99增加连击技能释放几率("..tostring(PetStar).."星套效果)"}
				PetEqYCSX[3] = {"#r#c33cc99外功防御 +"..tostring(PetStar*411).."","#r#c33cc99内功防御 +"..tostring(PetStar*411).."","#r#c33cc99血上限 +"..tostring(PetStar*764).."","#r#c33cc99增加灵气技能对玩家的生效几率("..tostring(PetStar).."星套效果)"}
...
if ePos ~= nil then
SuperTooltip2_Explain:SetText(szExplain.."#G宝石修炼评分 +"..BSdjpf.."\n#G宝石进阶评分 +"..BSdjpf);
else
...
function GetSe_XWoH(szPropertys,typeDesc,szAuthor)
	wunhPos1,wunhsxPos1 = string.find(typeDesc, "武魂")
	if  wunhPos1 ~= nil and  wunhsxPos1 ~= nil then
		local u1,u2,u3 =GetWuHunSttr(szAuthor)
		local tmp1,tmp2=string.find(szPropertys, "绑定#r")
		local tmp3,tmp4=string.find(szPropertys, "刻铭#r")
		if tmp3~=nil then
...
return "#cff6633成长率:900(完美)\n","#cff6633合成等级:"..hc.."\n","#cff99ff扩展属性栏数:"..kz.."\n#cffcc00"..sxtablestr..sxtablestr1..sxsx.."\n",sxtable
end

function GDisplEQJTInfoo(szAuthor)
	
	local JTtableE={}
	JTtableE["XS"]={"血上限",1000}
	JTtableE["LL"]={"力量"	,10}
	JTtableE["LQ"]={"灵气"	,10}
	JTtableE["TL"]={"体力"	,10}
	JTtableE["DL"]={"定力"	,10}
	JTtableE["SF"]={"身法"	,10	}
	JTtableE["WF"]={"外功防御",100}
	JTtableE["NF"]={"内功防御",	100}
	JTtableE["SB"]={"闪避"	,80}
	--------------------------------
	JTtableE["MZ"]={"命中"	,100}
	JTtableE["BG"]={"冰攻"	,15}
	...
	JTtableE["XG"]={"玄攻"	,15}
	JTtableE["DG"]={"毒攻"	,15	}
	JTtableE["WG"]={"外功攻击",	200}
	JTtableE["NG"]={"内功攻击"	,200	}
	
	local JTstr1,JTstr2,JTstr3 = "","",""
	if 	szAuthor ==nil or szAuthor =="" then return 0,0,0,0,0,0  end
	local x,y,JTstr1,JTun1,JTstr2,JTun2,JTstr3,JTun3= string.find(szAuthor,"&JT(%L%L)(%d%d)(%L%L)(%d%d)(%L%L)(%d%d)")
	if x==nil or y==nil then return 0,0,0,0,0,0  end
	JTstr1 =JTtableE[JTstr1][1].."(".. tonumber(JTun1).."级) +"..(tonumber(JTun1)*JTtableE[JTstr1][2]).."(共"..(99*JTtableE[JTstr1][2])..",".."已解封"..tonumber(JTun1).."%%)"
	JTstr2 =JTtableE[JTstr2][1].."("..tonumber(JTun2).."级) +"..(tonumber(JTun2)*JTtableE[JTstr2][2]).."(共"..(99*JTtableE[JTstr2][2])..",".."已解封"..tonumber(JTun2).."%%)"
	JTstr3 =JTtableE[JTstr3][1].."("..tonumber(JTun3).."级) +"..(tonumber(JTun3)*JTtableE[JTstr3][2]).."(共"..(99*JTtableE[JTstr3][2])..",".."已解封"..tonumber(JTun3).."%%)"
	return 1,1, JTstr1,JTstr2,JTstr3
end
...
function DisplayLPInfoo(strszAuthor)
	local Operating = {}
	Operating["a"]={"#cFF0000#895朱雀宝珠·血上限" ,   38001000,"血上限 +",12359,2359,800} --血  1为str ,2为物品ID，3为str ,4为官方参数，5为基本数，6为进阶数
	...
	Operating["g"]={"#G#897青龙宝珠·冰攻击" ,         38001006,"冰攻 +",172,22,5} --冰
	Operating["h"]={"#G#897青龙宝珠·火攻击" ,         38001007,"火攻 +",172,22,5}--火
	Operating["i"]={"#G#897青龙宝珠·玄攻击",          38001008,"玄攻 +",172,22,5} --玄
	Operating["j"]={"#G#897青龙宝珠·毒攻击" ,         38001009,"毒攻 +",172,22,5} --毒
	...
	if strszAuthor==nil then
		DWszAuthor = SuperTooltips2:GetAuthorInfo();
	else
	...
	jinjie_dj =tonumber( string.sub(lp_str0, 1,1))      ; ---君奉天原创独家——这是进阶数
	star_dj =tonumber (string.sub(lp_str0, 2,2)); ---君奉天原创独家——这是星星数
	...
	local lp_str = {}
	lp_str[1] =string.sub(lp_str1, 1,1); ---君奉天原创独家——这是切出来的类型
	lp_str[2] =string.sub(lp_str1, 4,4);
	lp_str[3] =string.sub(lp_str1, 7,7);
	lp_str[4] =string.sub(lp_str1, 10,10);
	lp_str[5] =string.sub(lp_str1, 2,3);---君奉天原创独家——这是切出来的等级
	...
	for i=1, tonumber(jinjie_dj)  do
		...
		if i>=2  and  i<=4 then
			local kuozhan_type = lp_str[i+7]  --君奉天原创独家——扩展类型
...
function DisplayLWInfo2(strszAuthor)
	local LWszAuthor
	if strszAuthor==nil then
		LWszAuthor = SuperTooltips2:GetAuthorInfo();
	else
		LWszAuthor = strszAuthor
	end
	local lvshux = {40,60,80,120,160,220,280,340,420,500} ------君奉天原创独家——属性
	local lvshux1 = {2,3,5,7,10,13,17,21,25,30} ---------君奉天原创独家——血上限，君奉天原创独家——减抗下限
	local xuestr = {"+2%","+3%","+4%","+5%","+7%","+9%","+11%","+14%","+17%","+20%"}
	local cz_index = {1,1,2,2,3,5,8,13,21,34} ---------君奉天原创独家——成长参数
...
-- 清空显示文本
--
function GetBaoshiStrrBB(baoshipfStr,nGemCounts)
...
function SuperTooltip2_ClearText()
	SuperTooltip2_StaticPart_Item6:SetText("")
...
function SuperTooltip2_OnMoved()
	
	local posX = SuperTooltip2_Frame:GetProperty("AbsoluteXPosition")
	local posY = SuperTooltip2_Frame:GetProperty("AbsoluteYPosition")
	local nWidth = SuperTooltip2_Frame:GetProperty("AbsoluteWidth")
	--SuperTooltips2:Cmp2WindowMainMove(tonumber(posX) , tonumber(posY) , tonumber(nWidth))
	
end

function SuperTooltip2_OnHide()
	--SuperTooltips2:CloseCmp2WindowMain()
	--SuperTooltips2:CloseCmp2WindowSub()
end